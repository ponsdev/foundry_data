import { actorDataConverter, actorTokenConverter, convertDistance, convertStringFromImperialToMetric, convertText, convertValueToMetric, relinkText } from "../Utils/ConversionEngineNew.js";
    ...
    switch (type) {
        case 'Actor5e':
            for (const item in entity.items) {
                if (!entity.items.hasOwnProperty(item))
                    continue;
...
const relinkCompendiums = async () => {
    ...
    for (const compendium of compendiums)
        if (compendium.includes('metrified'))
            await relinkCompendium(compendium, cache);
};
