import { actorDataConverter, actorTokenConverter, convertDistance, convertStringFromImperialToMetric, convertText, convertValueToMetric, labelConverter, } from "../Utils/ConversionEngineNew.js";
import { createErrorMessage } from "../Utils/ErrorHandler.js";
import { createNewCompendium, relinkCompendiums, typeSelector } from "./Compendium5eConverter.js";
import Utils from "../Utils/Utils.js";
const itemUpdater = async (item, onlyLabel, onlyUnit) => {
    if (item.getFlag("Foundry-MGL", "converted"))
        return;
    ...
    if (item.labels.range)
        item.labels.range = labelConverter(item.labels.range);
    try {
...
const itemsUpdater = async (items, onlyLabel, onlyUnit) => {
    for (const item of items)
        await itemUpdater(item, onlyLabel, onlyUnit);
};
...
const batchCompendiumUpdater = (compendiums) => async () => {
    for (const compendium of compendiums)
        if (!compendium.includes('metrified'))
            await compendiumUpdater(compendium);
