import { actorUpdater, itemUpdater, journalUpdater, rollTableUpdater, compendiumUpdater, batchCompendiumUpdater } from "./Dnd5e/Dnd5eConverterNew.js";
import { initBatchConversion } from "./Dnd5e/BatchConversion.js";
class MetricModule {
    constructor() {
    }
    static getInstance() {
        if (!MetricModule._instance)
            MetricModule._instance = new MetricModule();
    ...
    static addButton(element, actor, type) {
        ...
        button.on('click', () => {
            ...
            switch (type) {
                case 'actor':
                    actorUpdater(actor).then(() => ui.notifications.info(`Metrification complete, enjoy a better ${type}`));
                    break;
                case 'item':
                    itemUpdater(actor).then(() => ui.notifications.info(`Metrification complete, enjoy a better ${type}`));
                    break;
                case 'sheet':
                    journalUpdater(actor).then(() => ui.notifications.info(`Metrification complete, enjoy a better ${type}`));
                    break;
                case 'rolltable':
                    rollTableUpdater(actor).then(() => ui.notifications.info(`Metrification complete, enjoy a better ${type}`));
                    break;
                case 'compendium':
                    compendiumUpdater(actor).then(() => ui.notifications.info(`Metrification complete, enjoy a better ${type}`));
                    break;
            }
        });
    ...
    _createWarningDialog(callFunction) {
        new Dialog({
            title: 'Warning!',
    ...
    onRenderRollTable(obj, html) {
        let element = html.find(".window-header .window-title");
        MetricModule.addButton(element, obj.object, 'rolltable');
    }
    onCompendiumRender(obj, html) {
        let element = html.find(".window-header .window-title");
