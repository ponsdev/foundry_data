...
const imperialReplacer = (toReplace, replaceRegex) => toReplace.replace(replaceRegex, (element, value, unit) => {
...
const detailsConverter = (details) => {
    ...
    if (details.appearance)
        details.appearance = convertText(details.appearance);
    if (details.bond)
        details.bond = convertText(details.bond);
    if (details.flaw)
        details.flaw = convertText(details.flaw);
    if (details.ideal)
        details.ideal = convertText(details.ideal);
    if (details.trait)
        details.trait = convertText(details.trait);
    return details;
};
const actorDataConverter = (data) => {
    ...
    if (data.attributes.speed)
        data.attributes.speed = speedConverter(data.attributes.speed);
...
const relinkText = async (text, cache) => {
    ...
    for (const match of matched) {
        ...
        if (source.includes('metrified') || !game.packs.get(target))
            continue;
...
export { convertValueToMetric, convertStringFromImperialToMetric, isMetric, imperialReplacer, convertText, actorDataConverter, actorTokenConverter, convertDistance, labelConverter, relinkText };
