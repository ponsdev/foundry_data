...
const compositions = () => {
    const composites = {};
    for (const ten in tens)
        for (const digit in digits)
            composites[`${ten} ${digit}`] = tens[ten] + digits[digit];
    for (const bigTen in bigTens)
        for (const digit in digits)
            composites[`${digit} ${bigTen}`] = digits[digit] * bigTens[bigTen];
    return composites;
};
const allTheNumbers = {};
const numbersMerger = () => {
...
const numberSelector = (word1, word2) => {
    ...
    for (const numbers in allTheNumbers)
        if (final[0] === numbers) {
...
const numberToWords = (number) => {
    numbersMerger();
    for (const numbers in allTheNumbers)
        if (allTheNumbers[numbers] === number)
            return numbers;
    return number;
};
