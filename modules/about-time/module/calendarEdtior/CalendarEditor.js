...
export class CalendarEditor extends FormApplication {
    ...
    async _updateObject(event, formData) {
        ...
        this.object.yearNames = formData["yearname"];
    ...
    activateListeners(html) {
        ...
        html.find('.yearName-create').click(async (ev) => {
            if (!this.object.yearNames)
                this.object.yearNames = [];
            let newYearName = "New Year " + this.object.yearNames.length;
            this.object.yearNames.push(newYearName);
            this.render(false);
        });
        html.find('.yearName-delete').click(async (ev) => {
            if (this.object.yearNames.length <= 0)
                return; // must have one weekday
            let li = $(ev.currentTarget).parents(".yearName");
            let id = Number(li.attr("yearName-id"));
            this.object.yearNames.splice(id, 1);
            li.slideUp(200);
            this.render(false);
        });
