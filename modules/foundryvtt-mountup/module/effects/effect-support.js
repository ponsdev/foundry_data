import { duplicateExtended, i18n, isStringEquals, is_real_number } from "../lib/lib.js";
import Effect, { Constants } from "./effect.js";
export class EffectSupport {
    ...
    static convertActiveEffectToEffect(effect) {
        ...
        const changes = effect.data.changes.filter((change) => !change.key.startsWith('ATL') && change.key !== 'macro.tokenMagic' && !change.key.startsWith('ATCV'));
        ...
        return new Effect({
            ...
            description: i18n(effect.data.flags.customEffectDescription),
    ...
    //   return changes;
    // }
    static retrieveChangesOrderedByPriorityFromAE(effectEntity) {
        // Organize non-disabled effects by their application priority
        const changes = [effectEntity].reduce((changes, e) => {
            if (e.data.disabled) {
                return changes;
            }
    ...
    static prepareOriginForActor(actorOrAcotrId) {
        let actor;
        if (typeof actorOrAcotrId === 'string' || actorOrAcotrId instanceof String) {
            actor = game.actors?.get(actorOrAcotrId);
        }
        else {
    ...
    dimSight, brightSight, sightAngle, dimLight, brightLight, lightColor, lightAlpha, lightAngle, lightColoration = null, lightLuminosity = null, lightGradual = null, lightSaturation = null, lightContrast = null, lightShadows = null, lightAnimationType, lightAnimationSpeed, lightAnimationIntensity, lightAnimationReverse, 
    // applyAsAtlEffect = false, // rimosso
    effectName = null, effectIcon = null, duration = null, 
    // vision = false,
    // id: string | null = null,
    // name: string | null = null,
    height = null, width = null, scale = null) {
        const atlChanges = [];
        if (height && height > 0) {
            atlChanges.push({
