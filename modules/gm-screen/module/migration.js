...
export async function _gmScreenMigrate() {
    ...
    if (migrated.status) {
        ...
        if (isNewerVersion(game.modules.get(MODULE_ID).data.version, NEEDS_MIGRATION_VERSION))
            return;
    ...
    let gmScreenConfig = game.settings.get(MODULE_ID, MySettings.gmScreenConfig);
