...
import { createDamageList, getTraitMult, calculateDamage, addConcentration } from "./utils.js";
...
export let processpreCreateBetterRollsMessage = (data, options, user) => {
    ...
    if (!workflow) { // not doing the item.roll() TODO remove this when version .13 is out
        ...
        if (!item)
            item = game.items.get(brFlags.itemId);
