...
import { configSettings, enableWorkflow, criticalDamage, checkRule } from "./settings.js";
...
;
export function doCritModify(result, criticalModify = criticalDamage) {
    if (criticalModify === "default")
        return result;
    ...
    if (criticalModify === "maxDamage") { // max base damage
        ...
        if (rollBase.terms[0].number)
            rollBase.terms[0].number = rollBase.terms[0].number - bonusDice;
        //@ts-ignore .terms not defined
        ...
        //@ts-ignore .terms not defined
        if (rollCrit.terms[0].number)
            rollCrit.terms[0].number = rollCrit.terms[0].number - bonusDice;
        //@ts-ignore .terms not defined
        if (rollBase.terms[0].number)
            rollBase.terms[0].number = rollBase.terms[0].number - bonusDice;
        //@ts-ignore .terms not defined
