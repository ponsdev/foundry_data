...
export class Workflow {
    ...
    async callMacros(macros, tag) {
        ...
        for (let hit of this.hitTargets)
            hitTargets.push(hit.data);
        for (let failed of this.failedSaves)
            failedSaves.push(failed.data);
        for (let save of this.superSavers)
            superSavers.push(save.data);
        const macroData = {
    ...
    setRangedTargets(targetDetails) {
        ...
        canvas.tokens.placeables.filter(target => target.actor && target.actor.data.data.details.race !== "trigger"
