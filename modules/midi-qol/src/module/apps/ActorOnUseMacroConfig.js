...
import { getCurrentMacros, OnUseMacro, OnUseMacros } from "./Item.js";
export class ActorOnUseMacrosConfig extends FormApplication {
	...
	async getData(options) {
		...
		if (data.onUseMacroName !== undefined)
			data.onUseMacroParts = new OnUseMacros(data.onUseMacroName).items;
		else
			data.onUseMacroParts = new OnUseMacros(null).items;
