...
export function setupMidiTests() {
	if (!game?.user?.isGM)
		return;
	if (game.world.data.title !== "midi tests - quench")
		return;
		...
		globalThis.quench.registerBatch("quench.midi-qol.itemRolls", (context) => {
			...
			describe("Item Roll Tests", async function () {
				...
				it("applies cub conditions", async function () {
					...
					await busyWait(0.5);
					assert.ok(cubInterface.hasCondition("Blinded", [target]));
					const effect = target?.actor?.effects.find(e => e.data.label === "Cub Test");
				...
				it("applies CE conditions", async function () {
					...
					const effect = target?.actor?.effects.find(e => e.data.label === "CE Test");
					results = await target?.actor?.deleteEmbeddedDocuments("ActiveEffect", [effect?.id ?? "bad"]);
					await busyWait(0.1);
					if (await ceInterface.hasEffectApplied("Deafened", target?.actor?.uuid)) {
			...
			describe("Macro Roll Tests", async function () {
				it("runs macro execute", async function () {
					...
					assert.ok(target);
					try {
						let hasEffect = actor.effects.filter(a => a.data.label === "Macro Execute Test") ?? [];
						if (hasEffect?.length > 0)
							actor.deleteEmbeddedDocuments("ActiveEffect", hasEffect.map(e => e.id));
						hasEffect = target?.actor?.effects.filter(a => a.data.label === "Macro Execute Test") ?? [];
						if (hasEffect?.length > 0)
							target?.actor?.deleteEmbeddedDocuments("ActiveEffect", hasEffect.map(e => e.id));
						game.user?.updateTokenTargets([target?.id ?? ""]);
						await completeItemRoll(actor.items.getName("Macro Execute Test"));
						let flags = actor.data.flags["midi-qol"];
						...
						let hasEffect = target?.actor?.effects.filter(a => a.data.label === "Macro Execute Test") ?? [];
						await target?.actor?.deleteEmbeddedDocuments("ActiveEffect", hasEffect.map(e => e.id));
			...
			describe("onUse Macro Tests", async function () {
				it("Calls actor onUseMacros", async function () {
					...
					let hasEffects = actor.effects.filter(a => a.data.label === "OnUseMacroTest") ?? [];
		...
		globalThis.quench.registerBatch("quench.midi-qol.conditionImmunity", (context) => {
			...
			describe("Condition Immunity Tests", async function () {
				it("Tests condition immunity disables effect", async function () {
					...
					const theEffect = actor.effects.find(ef => ef.data.label === "Paralyzed");
					assert.ok(theEffect);
